...
class TaskParametersUtility {
    ...
    static _getAdditionalArgumentsTelemetry(additionalArguments, deploymentType) {
        const telemetry = {
        ...
        parsedArgs.forEach(function (arg) {
            ...
            for (let i = 0; i < arg.length; i++) {
                ...
                if (char == separator && equalsSignEncountered) {
                    ...
                    if (formattedArg.startsWith('"') && formattedArg.endsWith('"'))
                        telemetry.doubleQuoteCount++;
                    if (formattedArg.startsWith("'") && formattedArg.endsWith("'"))
                        telemetry.singleQuoteCount++;
                    if (escapedChars.test(formattedArg))
                        telemetry.escapeCharCount++;
                    if (/\s+/.test(formattedArg))
                        telemetry.spaceCharCount++;
                    telemetry.totalArgs++;
                    formattedArg = '';
                    continue;
                }
     ...
     */
    static parseAdditionalArguments(additionalArguments) {
        var parsedArgs = [];
